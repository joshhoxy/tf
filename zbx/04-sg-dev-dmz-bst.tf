...
resource "aws_security_group_rule" "sgr-dev-dmz-pub-bst-3" {
  ...
  cidr_blocks       = ["0.0.0.0/0"]
  security_group_id = aws_security_group.sg-dev-dmz-pub-bst-1.id
  description       = "From Public for HTTP (temp)"
}

resource "aws_security_group_rule" "sgr-dev-dmz-pub-bst-4" {
  # provider = aws.snm-dev
  type              = "ingress"
  from_port         = -1
  to_port           = -1
...
resource "aws_security_group_rule" "sgr-dev-dmz-pub-bst-5" {
  # provider = aws.snm-dev
  type              = "ingress"
  from_port         = 443
  to_port           = 443
  ...
  cidr_blocks       = ["172.16.0.0/22"]
  security_group_id = aws_security_group.sg-dev-dmz-pub-bst-1.id
  description       = "From JOSH-OTL-DMZ-VPC for HTTPS"
}